 ...
 */
package org.geocent.grid.fileParsers.netcdf;
...
import org.geocent.geotools.GrdDataEncapsulator;
...
public class NetCDFFileInspectorTest {
     ...
     * date. will read date from global attribute.
     */
    @Test
    ...
    /**
     * test that we can open and access NetcdfFile object created from both nc
     * and ncml files with the same file opening code.
     * 
     * TODO: evaluate whether we want to change our code to create NetcdfDataset
     * objects instead of NetcdfFile objects throughout our code.
     */
    @Test
    ...
    /**
     * test that we can open and access NetcdfDataset object created from both
     * nc and ncml files with the same dataset opening code.
     */
    @Test
    ...
    /**
     * test for ncml file doing aggregation of nc files with single run and
     * multiple taus per file.
     */
    @Test
    ...
    /**
     * test for ncml file doing aggregation of nc files with single tau per
     * file.
     */
    @Test
    ...
    /**
     * test that if we have two model runs, and we do not specify which run we
     * want data from, and the requested time is in both, we will get the
     * results from the later run.
     */
    @Test
     ...
     * 
     * This test passed for the previously existing 1D range read strategy (one
     * read per lat, i.e. get all lon values for a single lat in a single read).
     * This read style worked fine whether lat or lon is outer.
     * 
     * When I created the first implementation of the new 2D range read strategy
     * (one read for all lats and lons of interest) this test failed, as
     * expected, if the input file had variable with lon dim outer of lat. Test
     * passed once added separate branch for that case.
     */
    @Test
